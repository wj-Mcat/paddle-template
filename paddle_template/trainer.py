...
from paddle_template.config import Config, Tensor, get_logger
from paddle_template.data_processors import DataProcessor
from paddle_template.schema import ExampleDataset, InputExample
...
class Trainer:
    ...
    def create_dataloader(self, examples: List[InputExample]):
        ...
        return paddle.io.DataLoader(
            ...
            collate_fn=lambda _examples: self.convert_examples_to_features(
                _examples, dataset.label2idx
            ),
            return_list=True,
        )

    def convert_examples_to_features(
        self, examples: List[InputExample], label2idx: Dict[str, int]
    ):
        texts = []
        ...
        labels = paddle.to_tensor(
            [label2idx[example.label] for example in examples],
    ...
    def evaluate(self, dataloader: DataLoader, mode: str = "dev"):
        ...
        for features in dataloader:
            ...
            with paddle.amp.auto_cast(
                self.config.use_amp, custom_white_list=["layer_norm", "softmax", "gelu"]
            ):
            ...
            sub_acc = sum(
                [pred_index[index] == labels[index] for index in range(len(labels))]
            ) / len(labels)

            progress_bar.update()
            progress_bar.set_description(f"loss: {loss:10.4f} acc: {sub_acc: 10.4f}")

        # 2. compute the metric
        assert len(predicted) == len(truth)
        acc = sum([predicted[index] == truth for index in range(len(predicted))]) / len(
            predicted
        )
        ...
        self.writer.add_scalar(
            tag="eval-acc", value=acc, step=self.context_data.dev_step
        )
