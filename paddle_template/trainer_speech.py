...
from paddlenlp.transformers import BertTokenizer, PretrainedTokenizer
...
class TrainerSpeech(Trainer):
    def __init__(
        ...
        lr_scheduler = CosineAnnealingDecay(
            config.learning_rate, T_max=10, last_epoch=100
        )
        super().__init__(
            config, processor, tokenizer, model, criterion, lr_scheduler=lr_scheduler
        )
...
def main():
    ...
    model = SoundClassifier(
        cnn14(pretrained=True, extract_embedding=True), num_class=len(ESC50.label_list)
    )
